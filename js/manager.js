...
var checkList, checkUserSubscription, cview, find8comicOtherData, onAlarm, onInit, onLikeButton, scheduleRequest, startRequest;

onLikeButton = function(request, sender, sendResponse) {
  var e, ele, i, j, targetList, _i, _j, _len, _len1;
  console.log('onMessage', request);
  targetList = [];
  switch (request.params.site) {
    case 'dm5':
      targetList = localStorage.userDm5List != null ? JSON.parse(localStorage.userDm5List) : [];
      break;
    case '8comic':
      targetList = localStorage.user8comicList != null ? JSON.parse(localStorage.user8comicList) : [];
  }
  ...
  switch (request.action) {
    ...
    case 'clickLikeButton':
      ...
      for (i = _j = 0, _len1 = targetList.length; _j < _len1; i = ++_j) {
        ele = targetList[i];
        if (ele.menuUrl === request.params.menuUrl) {
          targetList = (function() {
            var _k, _len2, _results;
            _results = [];
            for (j = _k = 0, _len2 = targetList.length; _k < _len2; j = ++_k) {
              e = targetList[j];
              if (i !== j) {
                _results.push(e);
              }
