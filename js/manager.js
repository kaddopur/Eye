...
var checkList, cview, find8comicOtherData, find99770OtherData, findSfacgOtherData, onAlarm, onInit, onLikeButton, scheduleRequest, startRequest, sync;
...
startRequest = function(params) {
  ...
  $.get('http://comic.sfacg.com/WeeklyUpdate/', function(response) {
  ...
  return $.get('http://mh.99770.cc/comicupdate/', function(res) {
    var menuUrl, target, _i, _len, _ref, _results;
    _ref = $(res).find('.cpitem');
    _results = [];
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      target = _ref[_i];
      menuUrl = $(target).find('.t2 a').attr('href');
      _results.push(find99770OtherData(menuUrl));
    }
    return _results;
  });
};

find99770OtherData = function(menuUrl) {
  return $.get(menuUrl, function(response) {
    var comicName, edge, edgeNumber, edgeUrl, newBundle;
